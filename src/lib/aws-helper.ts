...
export const uploadToSignedUrl = async ({
  ...
  return new Promise((resolve, reject) => {
    ...
    Object.keys(signedUploadUrl.fields).forEach((key) => formData.append(key, signedUploadUrl.fields[key]!));
...
export const downloadFileFromS3 = async (bucketName: string, fileName: string) => {
...
export const uploadFiletoS3 = async (bucketName: string, fileName: string, file: Buffer) => {
  const params = {
    Bucket: bucketName,
    Key: fileName,
    Body: file,
...
export const s3OneTimeDownload = async (bucketName: string, fileName: string, expiration = 7) => {
  const params = {
    Bucket: bucketName,
    Key: fileName,
    Expires: expiration,
...
export function getPresignedPost({ params }: GetSignedUrlInput): Promise<PresignedPost> {
